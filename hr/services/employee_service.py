...
def create_employee(data: dict) -> Employee:
    ...
    # ── SECURITY STEP ────────────────────────────────────────────────────────
    # Do NOT generate or assign a password.
    # create_user() with no password= argument creates a valid user account
    # but with an unusable password — the employee cannot log in until they
    # set their own password via the setup link we email them below.
    ...
    user.set_unusable_password()   # makes it explicit and immune to blank-string login
    user.save()
    # ── END SECURITY STEP ────────────────────────────────────────────────────
...
def bulk_create_employees_from_file(uploaded_file) -> dict:
    ...
    for row_number, row in rows:
        try:
            ...
            record_lifecycle_event(employee, "hired", "Joined via bulk upload.")
            created.append({
...
def can_supervise(user) -> bool:
    """Returns True only for pure supervisor role (not HR/Dean)."""
    return get_role_key(user) == ROLE_SUPERVISOR
...
# ── CHANGE 2 ──────────────────────────────────────────────────────────────────
# send_employee_credentials() no longer receives or emails a password.
# Instead it generates a one-time password-setup link using Django's built-in
# PasswordResetTokenGenerator (the same mechanism Django's own password-reset
# page uses — secure, time-limited, single-use).
#
# The employee clicks the link, lands on /accounts/reset/<uidb64>/<token>/,
# and sets their own password.  HR never sees any password at any point.
# ── END CHANGE 2 ──────────────────────────────────────────────────────────────
...
def _apply_employee_data(employee: Employee, data: dict) -> None:
    allowed = (
        ...
        "employee_id", "attendance_pin", "pin",
    )
    updated = False
...
def _read_employee_upload_rows(uploaded_file):
    ...
    if filename.endswith(".xlsx"):
        try:
            ...
            raise ValidationError(
                ...
                "Install openpyxl on the machine running this project."
            )
    ...
    raise ValidationError(
        "Invalid file format. Please upload a .csv or .xlsx spreadsheet file."
    )
